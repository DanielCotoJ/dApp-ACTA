      ...
      <div className="relative w-full aspect-[1.414/1] bg-[#0000FF] overflow-hidden">
        ...
        <div className="absolute top-[60%] left-[3%] w-[52%]">
          <div className="relative border-b border-white/60 w-full">
            {holderName && (
              <span className="absolute -translate-y-[140%] font-sans text-white text-[2vw] tracking-[0.12em] uppercase whitespace-nowrap">
                {holderName}
              </span>
            )}
          </div>
...
export default function Certificate(props: CertificateProps) {
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a] flex items-center justify-center p-4">
      <div className="w-full max-w-[1200px] flex flex-col gap-3 sm:gap-4">
        <div className="w-full">
          <CertificateCanvas holderName={props.holderName} year={derivedYear} />
        </div>

        {details.length > 0 && (
          <div className="w-full rounded-xl bg-black/80 border border-white/15 px-5 py-4 sm:px-6 sm:py-5 text-white">
            ...
            <div className="space-y-2 sm:space-y-2.5 text-xs sm:text-sm">
              {details.map((item) => (
                <div
                  key={item.label}
                  className="grid grid-cols-1 sm:grid-cols-[160px_minmax(0,1fr)] gap-1 sm:gap-3 border-t border-white/8 first:border-t-0 pt-2 first:pt-0"
                >
